...
from alawodi_channel import post_to_alawodi_channel
...
from permissions_manager import (
    ...
    grant_permission, revoke_permission, AVAILABLE_TOOLS,
...
from admin_panel import (
    ...
    get_post_channels_keyboard,
)
from image_upscaler import upscale_image_with_ai
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "📢 *قنواتنا المميزة*\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "▪ [أحمد نجيب](https://t.me/Ahmad_Naguib_Al) — تعليم وأمن سيبراني\n"
        "▪ [العوادي 10](https://t.me/alawodi_10) — نكت وعبارات وخلفيات AI"
    )
...
async def course_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text(
            "من فضلك أرسل رابط فيديو صحيح وسأحمّله لك فوراً 🎬"
        )
        return
        ...
        with open(video_path, "rb") as vf:
            await update.message.reply_video(
                ...
                caption=(
                    ...
                    f"📝 {ep['description']}\n\n"
                    "🔔 اشترك في قناتينا ليصلك باقي الحلقات!"
                ),
                parse_mode="Markdown",
                read_timeout=900,
                write_timeout=900,
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if st["state"] in ("upscaling", "user_upscaling"):
        status = await update.message.reply_text("🔍 جاري تحسين جودة الصورة بأعلى دقة ممكنة...")
        try:
            photo = update.message.photo[-1]
            photo_file = await context.bot.get_file(photo.file_id)
            image_bytes = await photo_file.download_as_bytearray()

            enhanced = upscale_image_with_ai(bytes(image_bytes))
            ...
            await update.message.reply_document(
                ...
                caption=(
                    ...
                    "🔬 رُفعت الدقة 4× مع أعلى جودة ممكنة\n"
                    "✨ تم تحسين الحدة والألوان والتباين\n"
                    "📌 دقة الإخراج: JPEG 99%"
                ),
                parse_mode="Markdown",
            )
    ...
    await update.message.reply_text(
        "من فضلك أرسل رابط فيديو لتحميله 🎬"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if st["state"] == "revoke_username":
        target = text.lstrip("@").strip()
        clear_state(user_id)
        from permissions_manager import get_user_permissions as gup
        existing = gup(target)
        if not existing:
            await update.message.reply_text(
                f"⚠️ المستخدم @{target} ليس لديه صلاحيات مسجّلة.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_back")]]),
            )
        else:
            revoke_permission(target)
            await update.message.reply_text(
                f"✅ *تم إزالة جميع صلاحيات @{target} بنجاح!*",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_back")]]),
            )
        return
    ...
    if permissions:
        ...
        await update.message.reply_text(
            "من فضلك أرسل رابط فيديو صحيح من يوتيوب أو تيك توك أو إنستغرام أو فيسبوك وسأحمّله لك فوراً 🎬"
        )
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ("admin_upscale", "user_upscale"):
        ...
        await query.edit_message_text(
            ...
            "✅ رفع الدقة 4× بأعلى جودة ممكنة\n"
            "✨ تحسين الحدة والألوان والتباين\n"
            "🔒 لا تغيير في محتوى الصورة",
            parse_mode="Markdown",
        )
        return
    ...
    if data in ("admin_prophets", "user_prophets"):
        ...
        await query.edit_message_text(
            ...
            "🎬 اختر نبياً لتوليد مسلسله الكرتوني 👇",
            parse_mode="Markdown",
    ...
    # Admin course
    if data == "admin_course":
        if not is_admin(user_id):
            await query.answer("غير مصرح.", show_alert=True)
            return
        rows = []
        ep_items = list(EPISODES.items())
        for i in range(0, len(ep_items), 4):
            row = []
            for num, ep in ep_items[i:i+4]:
                row.append(InlineKeyboardButton(f"{num}", callback_data=f"admin_ep_{num}"))
            rows.append(row)
        rows.append([InlineKeyboardButton("🔙 رجوع", callback_data="admin_back")])
        await query.edit_message_text(
            "📚 *كورس Sketchware بالعربي*\n\n"
            "اختر رقم الحلقة لتوليد الفيديو مباشرةً 👇",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(rows),
        )
        return

    # Admin episode generate from panel
    if data.startswith("admin_ep_"):
        if not is_admin(user_id):
            await query.answer("غير مصرح.", show_alert=True)
            return
        try:
            ep_num = int(data.split("_")[2])
        except (IndexError, ValueError):
            return
        if ep_num not in EPISODES:
            await query.answer("الحلقة غير موجودة.", show_alert=True)
            return
        ep = EPISODES[ep_num]
        await query.edit_message_text(
            f"🎬 *جاري إنشاء الحلقة {ep_num}:* {ep['title']}\n\n"
        ...
        async def ep_progress(msg):
            try:
                await context.bot.send_message(chat_id=query.message.chat_id, text=msg)
            except Exception:
                pass
        try:
            video_path = await generate_episode_video_async(ep_num, ep_progress)
            await context.bot.send_message(
                chat_id=query.message.chat_id,
                text=f"✅ الحلقة {ep_num} جاهزة!\n📤 جاري الإرسال...",
            )
            with open(video_path, "rb") as vf:
            ...
            await context.bot.send_message(
                chat_id=query.message.chat_id,
                text=f"❌ خطأ في إنشاء الفيديو:\n{str(e)[:300]}",
            )
        return

    # Admin post channels
    if data == "admin_post_channels":
        if not is_admin(user_id):
            await query.answer("غير مصرح.", show_alert=True)
            return
        await query.edit_message_text(
            "📢 *نشر رسائل القنوات*\n\n"
            "اختر القناة التي تريد النشر فيها:\n"
            "⚠️ لن تُكرَّر الرسائل السابقة تلقائياً.",
            parse_mode="Markdown",
            reply_markup=get_post_channels_keyboard(),
        )
        return

    # Post to Ahmad channel
    if data == "post_ahmad":
        if not is_admin(user_id):
            await query.answer("غير مصرح.", show_alert=True)
            return
        await query.edit_message_text("📢 جاري النشر في قناة أحمد نجيب...")
        try:
            count = await post_to_channel(context.bot)
            await query.edit_message_text(
                f"✅ تم النشر بنجاح في قناة أحمد نجيب!\n📨 عدد الرسائل المنشورة: {count}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_back")]]),
            )
        except Exception as e:
            await query.edit_message_text(
                f"❌ خطأ في النشر: {str(e)[:200]}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_back")]]),
            )
        return

    # Post to alawodi channel
    if data == "post_alawodi":
        if not is_admin(user_id):
            await query.answer("غير مصرح.", show_alert=True)
            return
        await query.edit_message_text("😂 جاري النشر في قناة العوادي 10...")
        try:
            count_al = await post_to_alawodi_channel(context.bot, count=10)
            await query.edit_message_text(
                f"✅ تم النشر بنجاح في قناة @alawodi_10!\n📨 عدد الرسائل المنشورة: {count_al}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_back")]]),
            )
        except Exception as e:
            await query.edit_message_text(
    ...
    if data == "post_both":
        if not is_admin(user_id):
            await query.answer("غير مصرح.", show_alert=True)
            return
        await query.edit_message_text("📢 جاري النشر في القناتين...")
        errors = []
        count_ahmad = 0
        count_alawodi = 0
        try:
            count_ahmad = await post_to_channel(context.bot)
        except Exception as e:
            errors.append(f"قناة أحمد نجيب: {str(e)[:100]}")
        try:
            count_alawodi = await post_to_alawodi_channel(context.bot, count=10)
        except Exception as e:
            errors.append(f"قناة العوادي 10: {str(e)[:100]}")

        result_text = (
            f"✅ تم النشر في القناتين!\n"
            f"📨 قناة أحمد نجيب: {count_ahmad} رسالة\n"
            f"😂 قناة العوادي 10: {count_alawodi} رسالة"
        )
        if errors:
            result_text += "\n\n⚠️ أخطاء:\n" + "\n".join(errors)
    ...
    if data.startswith("prophet_"):
        ...
        await query.edit_message_text(
            f"🎬 *جاري إنشاء مسلسل قصة {prophet['name']}*\n\n"
            "🎨 سيتم إنشاء مشاهد كرتونية مع شخصيات وخلفيات...\n"
            "⏳ هذا قد يستغرق 3–7 دقائق\n"
            "سأُبلّغك عند اكتمال كل مشهد ✔",
            parse_mode="Markdown",
        )
        ...
        try:
            ...
            await context.bot.send_message(
                chat_id=query.message.chat_id,
                text=f"✅ مسلسل قصة {prophet['name']} جاهز!\n📤 جاري الإرسال...",
            )
            with open(video_path, "rb") as vf:
                await context.bot.send_video(
                    chat_id=query.message.chat_id,
                    video=vf,
                    caption=(
                        f"📖 *مسلسل قصة {prophet['name']}*\n\n"
                        f"🎬 {prophet['title']}\n\n"
                        "✨ مسلسل كرتوني إسلامي للأطفال\n"
    ...
    if data == "admin_revoke":
        if not is_admin(user_id):
            await query.answer("غير مصرح.", show_alert=True)
            return
        set_state(user_id, "revoke_username")
        await query.edit_message_text(
            "🚫 *إزالة صلاحيات مستخدم*\n\n"
            "✍️ أرسل اسم المستخدم الذي تريد إزالة صلاحياته:\n"
            "مثال: @username",
            parse_mode="Markdown",
        )
        return
